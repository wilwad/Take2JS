...
class take2 {
    constructor(divPlayer1, divDeck, divPlayer2) {
        ...
        // src: https://www.zapsplat.com/?s=fail&post_type=music&sound-effect-category-id=
        this.audio = {
    ...
    removeCard (arr, background, moveToDeck) {
        ...
        console.log('removeCard called', match, 'at', foundIdx);

        if (foundIdx > -1) {
            var card = arr.splice(foundIdx, 1); // remove the card from arr

            if (moveToDeck) {
                this.players[this.characters.Deck].push(card[0]); // move card to deck
            }
        }
    };
    ...
    resetDeck () {
        ...
        console.log('resetDeck', ava.length);
    ...
    //@param image - input : 6_of_diamonds.png output: number=>6, type=>diamonds
        ...
        for (var idx = 0; idx < divs.length; idx++) {
            ...
            if (num1 == cnum || ctype == type1) {
                switch (num1){
                    ...
                    default:
                        validCards.push(div); // normal add
                }
